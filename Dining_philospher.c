...
{	
	...
	{
		printf ( " PHILOSPHER %d ------------- %f, max waiting time %f \n", i+1, waiting_time_averages [ i ], max_waiting_time[i] );
	}
	return 0;
}
...
{	
	...
	for ( int iter = 0; iter < MAX_ITER; iter++ )
	{
        ...
        if (time_of_wait > max_waiting_time[id]) {   //update max waiting time
            max_waiting_time[id] = time_of_wait;
        }
